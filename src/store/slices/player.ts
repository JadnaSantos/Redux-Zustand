...
import { featchCourses } from "../../lib/axios";
  ...
  reducers: {
    ...
    next: (state) => {
      ...
      if (nextLesson) {
        ...
        const nexModuleIndex = state.currentModuleIndex + 1;
        const nextModule = state.course?.modules[nexModuleIndex];

        if (nextModule) {
          state.currentModuleIndex = nexModuleIndex;
